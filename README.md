...
| Inspect Mode on | log capture surfaces | Inspect-mode read-only diagnostic policy |
...
3. **Per-turn `knownIds` registry.** Seeded from user text + page context + inspect logs; grown by every tool result; pre-flight refuses any UID the model can't justify having seen.
...
7. (Optional) Click the gear → switch provider, paste API key, enable Inspect Mode, set theme.
8. (Optional) Provide a Tavily API key for the `browse_web` tool.

### Inspect Mode

Toggle from the `+` menu when a DHIS2 page misbehaves and the useful details only show in DevTools:

1. Turn **Inspect** on.
2. Reload / reproduce the issue.
3. Ask the assistant what happened.

Inspect captures only the active tab while it's enabled. Chrome doesn't expose previous DevTools console history to extensions, so flip Inspect on **before** triggering the issue. The assistant reads the captured logs in `read_only` diagnostic mode and only suggests fixes — it won't run destructive actions purely on inspect findings.
...
| `debugger` | Inspect-mode log capture only — disabled when Inspect is off |
