...
(() => {
  ...
  const btnInspectPage = $('btn-inspect-page');
  ...
  const inspectIndicator = $('inspect-indicator');
  const inspectIndicatorText = $('inspect-indicator-text');
  const btnClearInspect = $('btn-clear-inspect');
  ...
  let inspectEnabled = false; // persistent capture toggle for the current tab
  let inspectLogCount = 0;
  let inspectUrl = null;
  ...
  function setupListeners() {
    ...
    chatInput.addEventListener('input', () => {
      btnSend.disabled = !chatInput.value.trim() && !pendingImageBase64 && !pendingWebBrowse && !inspectEnabled;
    ...
    btnClearWebBrowse?.addEventListener('click', () => {
      ...
      updateWebBrowseIndicator();
      btnSend.disabled = !chatInput.value.trim() && !pendingImageBase64 && !inspectEnabled;
    });
  ...
  function handleBackgroundMessage(msg) {
    ...
    switch (msg.type) {
      ...
      case 'INSPECT_STATUS':
        ...
        inspectLogCount = Number(msg.count || 0);
        inspectUrl = msg.url || inspectUrl;
        updateInspectIndicator();
        break;
      case 'AI_THINKING':
  ...
  function updateState(state) {
    currentState = state;
    if (state?.inspect) {
      inspectEnabled = !!state.inspect.enabled;
      inspectLogCount = Number(state.inspect.count || 0);
      inspectUrl = state.inspect.url || inspectUrl;
      updateInspectIndicator();
    }
    if (state.connected) {
  ...
  function updateInspectIndicator() {
    if (inspectIndicator) {
      inspectIndicator.classList.toggle('hidden', !inspectEnabled);
    }
    if (inspectIndicatorText) {
      const countText = inspectLogCount === 1 ? '1 log' : `${inspectLogCount} logs`;
      inspectIndicatorText.textContent = inspectEnabled ? `Inspect on - ${countText}` : 'Inspect off';
    }
    if (btnInspectPage) {
      btnInspectPage.classList.toggle('attach-option-active', !!inspectEnabled);
      btnInspectPage.title = inspectEnabled
        ? 'Stop capturing console and network errors'
        : 'Capture console and network errors for this tab';
    }
    btnSend.disabled = !chatInput.value.trim() && !pendingImageBase64 && !pendingWebBrowse && !inspectEnabled;
  }

  function refreshInspectStatus() {
    chrome.runtime.sendMessage({ type: 'GET_INSPECT_STATUS' }, (resp) => {
      if (chrome.runtime.lastError || !resp) return;
      inspectEnabled = !!resp.enabled;
      inspectLogCount = Number(resp.count || 0);
      inspectUrl = resp.url || null;
      updateInspectIndicator();
    });
  }
  ...
  function startInspectMode() {
    chrome.runtime.sendMessage({ type: 'START_INSPECT' }, (resp) => {
      if (chrome.runtime.lastError) {
        addMessage('error', chrome.runtime.lastError.message || 'Unable to start Inspect mode.');
        return;
      }
      if (!resp?.success) {
        addMessage('error', resp?.error || 'Unable to start Inspect mode.');
        return;
      }
      inspectEnabled = true;
      inspectLogCount = Number(resp.count || 0);
      inspectUrl = resp.url || null;
      updateInspectIndicator();
      addMessage('system', 'Inspect mode is on. Reproduce the issue in this DHIS2 tab, then ask what happened.');
    });
  }

  function stopInspectMode() {
    chrome.runtime.sendMessage({ type: 'STOP_INSPECT' }, (resp) => {
      if (chrome.runtime.lastError) {
        addMessage('error', chrome.runtime.lastError.message || 'Unable to stop Inspect mode.');
        return;
      }
      inspectEnabled = false;
      inspectLogCount = Number(resp?.count || inspectLogCount || 0);
      inspectUrl = resp?.url || inspectUrl;
      updateInspectIndicator();
    });
  }
  ...
  function sendMessage() {
    ...
    if ((!text && !pendingImageBase64 && !pendingWebBrowse && !inspectEnabled) || isSending) return;

    welcome.classList.add('hidden');
    const userPreview = text || (pendingImageBase64 ? '(image)' : pendingWebBrowse ? '(web browse)' : '(inspect logs)');
    ...
    const payload = { text: text || (pendingWebBrowse ? 'Browse the web and answer my question.' : inspectEnabled ? 'Inspect the captured page errors and explain what is happening.' : 'Analyze this image') };
    if (pendingImageBase64) {
      payload.imageBase64 = pendingImageBase64;
    }
    if (pendingWebBrowse) payload.browseWeb = true;
    if (inspectEnabled) payload.inspect = true;
    clearPendingImage();
    pendingWebBrowse = false;
    updateWebBrowseIndicator();
